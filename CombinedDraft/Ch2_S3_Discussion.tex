...
% paragraph to add
\todo{new} Talk about reasonableness of stepping down results to the project-level. When is that appropriate and when is it not? Plan to revise FRV section on same topic then port most of it back here. Since we do have equilibrium and long time scales in the HRV there is probably a little bit more to say. Lee points out that the box plots summarize over time and space. The average canopy cover map summarizes over time, but lets you see space. to inform project decisions, a lot of plots are current vs. historic or between different climate scenarios or a function of time. or ratio of early to late development. could also plot, over full time of hrv, do the average for some small area over time, of proportions or ratio. and you could make it much coarser grained, and you could make it the spatial average. Important to emphasize that such a map shouldn't be used to make management decisions, but is a way to prioritize survey and monitoring work to determine best place to do management actions, since actual actions should always be supported by on-the-ground evidence.
