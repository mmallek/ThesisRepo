...
In\todo{paragraph drafted but not finished! may also be good to make plots for all stages, even if not all are published or some are in an appendix} order to determine whether the results were an artifact of the initial condition, I also examined the trajectory of early seral composition for both mesic and xeric mixed conifer forests. Because the initial condition for both forest types was a very low amount of land in the early successional stage, it seemed plausible that the first few timesteps would have very high rates of wildfire and high mortality wildfire, leading to higher than otherwise expected quantities of land in the early successional stage. However, Figures~1&2, which depict the mean\todo{should this be median?} values across all runs of each climate model scenario, show that this is not in fact the case. The anticipated increase in early successional forest does occur in both forest types. However, in mesic mixed conifer the proportion dips back down, and appears to be responding more to the climate parameter than the initial conditions, particularly by timestep 14 when we begin using the output landscape grids for analysis. In xeric mixed conifer, the initial increase continues throughout the length of the simulation, again indicating that the driver behind this increase is related primarily to the climate setting and related feedback, rather than the initial conditions. Again, my analysis is focused on the end of the simulation in part to avoid incorporating artifacts related to initial conditions. These plots show that the amount of early seral mixed conifer forest is increasing over time during the simulation, and thus the short length of the simulation is not an overriding factor in the decline of middle and late successional forest.
