...
\footnotesize
\centering
\caption{List of seral stages developed for this project. Seral stages describe developmental stage (e.g. ``Early Development'') and canopy closure (e.g. ``open''). See the cover type descriptions in Appendix~\ref{app:covertypedesc} for more information on the metrics used to classify seral stages. The non-seral ``stage'' applies to land cover types for which I do not simulate succession (Barren, Grassland, Urban, Agriculture, Water, and Meadow). Included are the seral stage abbreviations, and full names.}
\label{condtable}
\begin{tabular}{@{}ll@{}}
\toprule
