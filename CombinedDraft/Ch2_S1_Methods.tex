...
The source for the cover layer is the Region 5 Existing Vegetation Map (``EVeg''), first mapped to the \textsc{calveg} classification developed by the Region's Ecology Program in 1978 \citep{USDAForestService2009}. When deciding on land cover types, including determining xeric and mesic subtypes, my focus was to best represent the study area and the surrounding landscape. I used the ``North Sierra'' \textsc{calveg} mapping zone boundary (Figure~\ref{calveg}) as the focus for defining vegetation and disturbance, including susceptibility, response to fire, and fire size and distribution \citep{USDAForestService2008}. Within the study area, the EVeg layer was developed based on three separate efforts: a satellite-based imagery analysis in 2000, and two orthoimagery analyses completed by contracting firms in 2005. Generally, specific cover type names were derived from the California Fire Return Interval Departure (FRID) report by \citet{VandeWater2011}. I also considered information from \emph{A Guide to Wildlife Habitats of California}, popularly known as the ``Wildlife Habitat Relationship'' (WHR) cover types \citep{WHR1988}. 
...
\subparagraph*{Selection of Specific Cover Types}\todo{confusing to not be consistent re: (land) cover types? ask lee}
...
In collaboration with project partners\todo{check for consistent reference to partners} I decided to instead base the land cover types on, at the first order, the Presettlement Fire Regime (PFR) types as defined in the Fire Return Interval Departure (FRID) report by \citet{VandeWater2011}. The PFR types, as part of the FRID, were developed through a combination of literature and expert workshops. Peer review was solicited during these workshops, and the framework was then subjected to additional review via the academic publication process. The PFR was also useful for this project specifically because it grouped vegetation types based on their relationship to wildfire, which is the disturbance type simulated in this study \citep{VandeWater2011}. Using the FRID methodology provided an opportunity to avoid using the second-order WHR classification and trying to reverse-engineer it to fit into my custom land cover types. 
...
\footnotesize
\centering
\caption{List of seral stages developed for this project. Seral stages describe developmental stage (e.g. ``Early Development'') and canopy closure (e.g. ``open''). The non-seral ``stage'' applies to land cover types for which I do not simulate succession (Barren, Grassland, Urban, Agriculture, Water, and Meadow). Included are the seral stage abbreviations, and full names.}
\label{condtable}
\begin{tabular}{@{}ll@{}}
\toprule
...
Transitions between Early and Middle Development, and between Middle and Late Development are governed by the time in the Early or Middle stage (\emph{Development-Age}). These transitions may begin at the minimum time in a specified \emph{Development-Age}, and proceed at rates that vary across cover types. Table~\ref{smcm_vegtrans} displays the average \emph{Seral Stage-Age} of transition. If a cell reaches the maximum stage-age listed, its probability of transitioning goes to 1. 
